...
impl ClientAdapter for PiAdapter {
    ...
    fn detect(&self) -> Vec<ConfigLocation> {
        ...
        // 1. Primary global: ~/.pi/agent/mcp.json
        if let Some(home) = dirs::home_dir() {
            let pi_agent_path = home.join(".pi").join("agent").join("mcp.json");
            let exists = pi_agent_path.exists();
            locs.push(ConfigLocation {
                client_id: self.id().to_string(),
                display_name: format!("{} (Agent Global)", self.display_name()),
                path: pi_agent_path,
                scope: Scope::Global,
                exists,
            });
        ...
        // 2. Standard config dir: ~/.config/mcp/mcp.json
        if let Some(config) = dirs::config_dir() {
            let mcp_config_path = config.join("mcp").join("mcp.json");
            let exists = mcp_config_path.exists();
            locs.push(ConfigLocation {
                client_id: self.id().to_string(),
                display_name: format!("{} (Config Global)", self.display_name()),
