...
#[derive(Default)]
pub struct ManicodeAdapter;

impl ManicodeAdapter {
    pub fn new() -> Self {
        Self
    }
}

impl ClientAdapter for ManicodeAdapter {
    ...
    fn detect(&self) -> Vec<ConfigLocation> {
        ...
        if let Some(config_dir) = dirs::config_dir() {
            let path = config_dir.join("manicode").join("mcp.json");
        ...
        if let Some(home) = dirs::home_dir() {
            let path = home.join(".manicode").join("mcp.json");
            let exists = path.exists();
            locs.push(ConfigLocation {
                client_id: self.id().to_string(),
        ...
        let project_path = PathBuf::from(".manicode").join("mcp.json");
